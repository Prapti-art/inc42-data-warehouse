...
Usage:
    ...
    python ingestion/scripts/woocommerce_ingest.py
"""
...
TABLES = {
    ...
    "woocommerce_order_meta": {
        ...
        """,
        "bq_table": "bronze.woocommerce_order_meta",
    },
...
def load_table(name, config):
    """Run query on MySQL and load result into BigQuery Bronze."""
    bq_table = config["bq_table"]
    query = config["query"]
    print(f"  {name} → {bq_table}...", end=" ")
...
def main():
    print("=" * 60)
    print("WOOCOMMERCE (MYSQL) → BIGQUERY BRONZE")
    print("=" * 60)
