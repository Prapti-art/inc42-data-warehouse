    ...
    create or replace table `bigquery-296406`.`silver_gold`.`fact_form_submissions`
...
WITH gravity_forms AS (
    ...
    FROM `bigquery-296406`.`bronze`.`gravity_forms`
),
...
tally_forms AS (
    ...
    FROM `bigquery-296406`.`bronze`.`tally_forms`
),
...
FROM all_forms f
LEFT JOIN `bigquery-296406`.`silver_gold`.`dim_contact` dc ON f.email = dc.email
    );
